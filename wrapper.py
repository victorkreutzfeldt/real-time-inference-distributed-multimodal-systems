...
parser = argparse.ArgumentParser(
    description="Wrapper for real-time inference with distributed multimodal systems."
)
...
if __name__ == "__main__":
    ...
    # Load test dataset
    test_ds = PerVideoMultimodalDatasetLabels(ANNOTATIONS_CSV, 'test', NUM_TOKENS, NUM_CLASSES)
    ...
    # Fallback feature for missing audio token
    ...
    results = {
        ...
        'curr_num_completed_tokens_audio': [[] for _ in range(num_test_videos)],
        'curr_num_completed_tokens_video': [[] for _ in range(num_test_videos)],
    ...
    for idx, video_id in tqdm(enumerate(test_videos), total=len(test_videos), desc="Aligned Async Inference", ascii=True):
        ...
        # Initalize number of complete and incomplete tokens
        num_complete_tokens_audio = 0
        num_incomplete_tokens_audio = NUM_TOKENS

        num_complete_tokens_video = 0
        num_incomplete_tokens_video = NUM_TOKENS
        ...
        while True:
            ...
            # Check if we have completed token information
            if PROCESS_AUDIO and out_audio['num_complete_tokens'] > 0: 
                audio_tokens_buffer.extend(out_audio['embs'][:out_audio['num_complete_tokens']])
                num_complete_tokens_audio += out_audio['num_complete_tokens']
                num_incomplete_tokens_audio = NUM_TOKENS - num_complete_tokens_audio

            if PROCESS_VIDEO and out_video['num_complete_tokens'] > 0: 
                video_tokens_buffer.extend(out_video['feats'][:out_video['num_complete_tokens']])
                num_complete_tokens_video += out_video['num_complete_tokens']
                num_incomplete_tokens_video = NUM_TOKENS - num_complete_tokens_video

            # Store current number of completed tokens
            results['curr_num_completed_tokens_audio'][idx].append(num_complete_tokens_audio)
            results['curr_num_completed_tokens_video'][idx].append(num_complete_tokens_video)

            # Get current partial complete token (if any)
            curr_partially_complete_audio = None
            if PROCESS_AUDIO and out_audio['num_exceeding_pkts'] > 0:
                curr_partially_complete_audio = out_audio['embs'][out_audio['num_complete_tokens']:out_audio['num_complete_tokens'] + 1]

            curr_partially_complete_video = None
            if PROCESS_VIDEO and out_video['num_exceeding_pkts'] > 0:
                curr_partially_complete_video = out_video['feats'][out_video['num_complete_tokens']:out_video['num_complete_tokens'] + 1]


            # Get current data - audio
            if PROCESS_AUDIO:
                if num_complete_tokens_audio > 0:
                    if curr_partially_complete_audio is not None:
                        curr_audio = np.concatenate((list(audio_tokens_buffer), curr_partially_complete_audio), axis=0)
                    else:
                        curr_audio = np.stack(audio_tokens_buffer, axis=0)
                else:
                    ...
                    if curr_partially_complete_audio is not None:
                        ...
                        curr_audio = np.empty((0, *audio_tokens_buffer[0].shape[1:]))  # TODO: adjust shape accordingly

            # Get current data - video
            if PROCESS_VIDEO:
                if num_complete_tokens_video > 0:
                    if curr_partially_complete_video is not None:
                        curr_video = np.concatenate((list(video_tokens_buffer), curr_partially_complete_video), axis=0)
                    else:
                        curr_video = np.stack(video_tokens_buffer, axis=0)
                else:
                    if curr_partially_complete_video is not None:
                        curr_video = curr_partially_complete_video
                    else:
                        curr_video = np.empty((0, *video_tokens_buffer[0].shape[1:]))  # TODO: adjust shape accordingly
           

            # ########## Handle Future Data ########## 


            # Get number of partially complete and complete tokens and complete the rest with fallback features
            if PROCESS_AUDIO:
                num_partially_complete_tokens_audio = (1 if out_audio['num_exceeding_pkts'] > 0 else 0)
            
                # Concatenate incomplete tokens
                if num_incomplete_tokens_audio > 0:
                    incomplete_tokens_audio_embs = np.tile(fallback_audio_token_embs, (num_incomplete_tokens_audio-num_partially_complete_tokens_audio, 1))
                    curr_audio = np.concatenate((curr_audio, incomplete_tokens_audio_embs), axis=0)
                else:
                    curr_audio = curr_audio

            if PROCESS_VIDEO:
                num_partially_complete_tokens_video = (1 if out_video['num_exceeding_pkts'] > 0 else 0)
            
                # Concatenate incomplete tokens
                if num_incomplete_tokens_video > 0:
                    incomplete_tokens_video_feat = np.tile(fallback_video_feat, (num_incomplete_tokens_video-num_partially_complete_tokens_video, 1, 1, 1))
                    curr_video = np.concatenate((curr_video, incomplete_tokens_video_feat), axis=0)
                else:
            ...
            with torch.no_grad():
                try:
                    ...
                    breakpoint() # TODO: something wrong here
            ...
            if num_complete_tokens_audio == NUM_TOKENS:
                PROCESS_AUDIO = False

            if num_complete_tokens_video == NUM_TOKENS:
                PROCESS_VIDEO = False
